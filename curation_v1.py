...
def create_job_new_style(jobs_client):
    ...
    # Add Hive credentials if present
    ...
    # -------------------- Submit Job --------------------
    resp = jobs_client.create_job(job_data_dict,workspace_id)
    return resp.get("job_id")

# -------------------- Job Monitoring --------------------
def monitor_job(jobs, run_id):
    while True:
        status = jobs.get_job_status(run_id,workspace_id)
        print("Status:", status)
...
if __name__ == "__main__":
    ...
    extra_args = " ".join(key_value_pairs)
    ...
    run_resp = jobs.run_job({"job_id": job_id},workspace_id)
    run_id = int(run_resp.get("run_id"))
    print("Run started, id:", run_id)

    # Monitor the job status until done
    monitor_job(jobs, run_id)
